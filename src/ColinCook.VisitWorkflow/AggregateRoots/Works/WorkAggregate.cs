...
namespace ColinCCook.AspNetCoreEventSourcing.EventFlow.AggregateRoots.Works
{
        ...
        IEmit<WorkCompletedEvent>
    {
        ...
        {
            if (_isComplete) return ExecutionResult.Failed("An operative cannot abandon work when it's already been completed");

            if (_operative == null) return ExecutionResult.Failed("There is no operative assigned to this work to abandon it");
        ...
        public IExecutionResult Complete()
        {
            ...
            if (_operative == null) return ExecutionResult.Failed("There must be an operative assigned to complete the work");
